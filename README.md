# MouseMine-search-R-
The code below was used to query the MouseMine for mouse genes associated with the phenotype ```prenatal lethality```, achieved by including constraints on the query. This method used the ```R``` library ```InterMineR```. Alternative model organisms can be selected, and available mines can be found via searching the below mentioned ```listMines()```. The process searched for a particular phenotype, put a constraint on the zygosity and requested publications are included in the results. This is the first of a series of repositories to search for homologs using annotated genes from the MouseMine and identify variants within these genes, and to search for flanking sequences using the rsIDs. This code was used for a reverse genetic screen approach.
